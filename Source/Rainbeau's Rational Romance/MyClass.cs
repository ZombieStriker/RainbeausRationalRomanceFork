		...
		public static void CurrentStateInternal(ref ThoughtState __result, Pawn p)
		{
			if (__result.StageIndex != ThoughtState.Inactive.StageIndex)
			{
				if (p.story.traits.HasTrait(RRRTraitDefOf.Polyamorous))
				{
					...
					if (directPawnRelation != null && p.ownership.OwnedBed != null && multiplePartners)
					{
						if (partnerBedInRoom)
						{
							__result = false;
						...
						else
						{
							...
							//Differnt type of check
							bool partnerBedInRoomSecondCheck = (from t in partners where p.CurrentBed().GetRoom().ContainedBeds.Contains(t.CurrentBed()) select t).Count() > 0;
							if (partnerBedInRoomSecondCheck)
								__result = false;
						}
					}
				}
